...
export function CatalogSearch({ initialValue }: CatalogSearchProps) {
  ...
  // Only sync with initialValue if it changed externally (e.g., browser back/forward)
  // and we don't have a pending search that matches
  useEffect(() => {
    const trimmedInitial = initialValue.trim();
    const pendingValue = pendingSearchRef.current?.trim() ?? null;

    // If initialValue changed and it's not from our pending search, sync it
    if (
      ...
      // Our pending search completed, clear it
      pendingSearchRef.current = null;
